...
# --- Windows: Task Scheduler (delegates to PowerShell script) ---
windows_install() {
    local ps_script="$PROJECT_DIR/scripts/setup-autostart.ps1"
    if [[ -f "$ps_script" ]]; then
        powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$ps_script" -Install
    else
...
windows_remove() {
    local ps_script="$PROJECT_DIR/scripts/setup-autostart.ps1"
    if [[ -f "$ps_script" ]]; then
        powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$ps_script" -Remove
    else
        echo "Warning: $ps_script not found"
    fi
}

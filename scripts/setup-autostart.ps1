...
function Install-AutoStart {
    Write-Host "Installing ACP daemon auto-start..."
    ...
    $wrapperLines = @(
        "# ACP daemon launcher (generated by setup-autostart.ps1)",
        "`$logFile = `"$logFile`""
    )
    ...
    # Create acp-remote.cmd in user PATH
    Install-RemoteCmd
}

function Remove-AutoStart {
    Write-Host "Removing ACP daemon auto-start..."
...
function Get-AutoStartStatus {
    try {
        ...
        Write-Host "ACP daemon: ENABLED (Task Scheduler)"
        Write-Host "  State: $($task.State)"
        Write-Host "  Next Run: $($task.NextRunTime)"
    } catch {
        Write-Host "ACP daemon: NOT INSTALLED"
    }
}

function Install-RemoteCmd {
    $binDir = Join-Path $env:USERPROFILE ".local" "bin"
    New-Item -ItemType Directory -Force -Path $binDir | Out-Null
    $cmdPath = Join-Path $binDir "acp-remote.cmd"

    $psScriptPath = Join-Path $Dir "scripts" "acp-remote.ps1"
    $cmdLines = @(
        "@echo off",
    ...
    Write-Host "  acp-remote command added to PATH: $cmdPath"
}

function Remove-RemoteCmd {
    $cmdPath = Join-Path $env:USERPROFILE ".local" "bin" "acp-remote.cmd"
    if (Test-Path $cmdPath) {
        Remove-Item $cmdPath -Force
        Write-Host "  acp-remote command removed"
    }
}

...
function Install-AutoStart {
    ...
    # Write sensitive env vars to a restricted file instead of inlining in scripts.
    $configDir = Join-Path $env:USERPROFILE ".config\runmote"
    New-Item -ItemType Directory -Force -Path $configDir | Out-Null
    $envFile = Join-Path $configDir "environment"
    @(
        if ($env:ACP_DAEMON_TOKEN)    { "ACP_DAEMON_TOKEN=$($env:ACP_DAEMON_TOKEN)" }
        if ($env:ACP_DAEMON_ID)       { "ACP_DAEMON_ID=$($env:ACP_DAEMON_ID)" }
        if ($env:ACP_RELAY_URL)       { "ACP_RELAY_URL=$($env:ACP_RELAY_URL)" }
        if ($env:ACP_RELAY_PUBLIC_URL) { "ACP_RELAY_PUBLIC_URL=$($env:ACP_RELAY_PUBLIC_URL)" }
    ) | Set-Content -Path $envFile -Encoding ASCII
    Write-Host "  environment written: $envFile"
    ...
    $wrapperLines = @(
        ...
        "# Load env vars from restricted file (avoids token leakage in process listing)",
        "if (Test-Path `"$userHome\.config\runmote\environment`") {",
        "    Get-Content `"$userHome\.config\runmote\environment`" | ForEach-Object {",
        ...
        "`$agentPaths = @(`"$userHome\.local\bin`", `"$userHome\AppData\Roaming\npm`", `"$userHome\.opencode\bin`", `"$userHome\.cargo\bin`", `"$userHome\.bun\bin`")",
        "`$env:Path = (`$agentPaths + `$env:Path.Split(';') | Where-Object { `$_ -and (Test-Path `$_ -ErrorAction SilentlyContinue) }) -join ';'",
        "Start-Process -WindowStyle Hidden -FilePath `"$python`" -ArgumentList `"-m`", `"src.daemon.main`" -WorkingDirectory `"$Dir`" -RedirectStandardOutput `$logFile -RedirectStandardError `$errFile"
    )
